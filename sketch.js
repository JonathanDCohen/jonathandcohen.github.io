...
class Rail {
  ...
  draw(floorX, ceilingX) {
    ...
    for (let vertex = firstVertex; vertex < lastVertex; ++vertex) {
      ...
      const deltaY =
        this.gridToPixel(endVertex.j) - this.gridToPixel(startVertex.j);

      const startX = clampedGridToPixel(startVertex.i);
      // The slope is 1
      const startY =
        this.gridToPixel(startVertex.j) +
        ((startX - this.gridToPixel(startVertex.i)) * deltaY) / this.stepSize;
      const endX = clampedGridToPixel(endVertex.i);
      const endY =
        this.gridToPixel(endVertex.j) -
        ((this.gridToPixel(endVertex.i) - endX) * deltaY) / this.stepSize;

      fill(this.h, this.s, this.b);
      quad(
        startX,
        startY,
        startX - this.railThickness * tan(PI / 8),
        startY + this.railThickness,
        endX - this.railThickness * tan(PI / 8),
        endY + this.railThickness,
        endX,
        endY
      );
    }
    ...
    for (let i = 0; i < numRails; ++i) {
      this.rails.push(
        new Rail(
          ...
          s,
          v,
          startI + (i / numRails) * this.bundleHeight,
          gridSizePixels,
          segmentSize,
          railThickness
        )
...
function newBundle() {
  ...
  const startI = random(0, (windowHeight / gridSizePixels) * 0.9);
  ...
  for (let i = 0; i < ceil(windowWidth / gridSizePixels / segmentSize); ++i) {
    bundle.addSegment(random() < 0.5 ? Direction.OVER : Direction.DOWN);
  }
  return bundle;
}
